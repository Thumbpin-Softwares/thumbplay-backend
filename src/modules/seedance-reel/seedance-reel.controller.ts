...
export async function generatePipeline(req: AuthedRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    const creditResult = await consumeCreditsForAction({
      ...
      metadata: { endpoint: '/api/v1/seedance-reel/generate-pipeline' },
    });
    if (!creditResult.ok) {
    ...
    try {
      ...
      await Promise.allSettled([
        callSeedanceAndUpload(part1Input, userId, 'sreel-part1', pipelineSignal)
          .then((url) => {
            ...
            send({ type: 'part1_video_done', part1VideoUrl: url, message: 'Part 1 (car exit intro) video ready!' });
      ...
      if (debit) {
        await refundCreditsForAction({
          ...
          action: CREDIT_ACTION,
          debit,
          metadata: { endpoint: '/api/v1/seedance-reel/generate-pipeline', reason: 'generation_failed', message },
        });
      }
    ...
    if (debit) {
      await refundCreditsForAction({
        ...
        action: CREDIT_ACTION,
        debit,
        metadata: { endpoint: '/api/v1/seedance-reel/generate-pipeline', reason: 'unexpected_error', message },
      });
    }
