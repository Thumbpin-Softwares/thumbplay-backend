...
export async function generatePipeline(req: AuthedRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    const creditResult = await consumeCreditsForAction({
      ...
      metadata: { endpoint: '/api/v1/comedy-reel/generate-pipeline' },
    });
    if (!creditResult.ok) {
    ...
    await Promise.all(
      locationBufs.map(async (buf, i) => {
        try {
          ...
          const key = buildUserKey(userId, 'images', 'jpg', `comedy-location-${i}`);
    ...
    try {
      ...
      const { part1, part2, part1Words, part2Words } = await splitScript(script, 'a fast-paced comedy vertical reel video');
      ...
      if (debit) {
        await refundCreditsForAction({
          ...
          action: CREDIT_ACTION,
          debit,
          metadata: { endpoint: '/api/v1/comedy-reel/generate-pipeline', reason: 'generation_failed', message },
        });
      }
    ...
    if (debit) {
      await refundCreditsForAction({
        ...
        action: CREDIT_ACTION,
        debit,
        metadata: { endpoint: '/api/v1/comedy-reel/generate-pipeline', reason: 'unexpected_error', message },
      });
    }
...
export const previewScript = createPreviewScriptHandler('ComedyReel');
